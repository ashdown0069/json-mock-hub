...
describe('MembershipService.removeMember', () => {
  const createService = (membership: Record<string, unknown> | null) => {
    ...
    const membershipModel = { findOne } as any;
  ...
  it('owner는 추방할 수 없다 — 워크스페이스가 영구히 잠기기 때문', async () => {
    const { service } = createService({ role: 'owner', save: jest.fn() });
  ...
  it('member는 soft delete하고 membersCount를 1 줄인다', async () => {
    const save = jest.fn().mockResolvedValue(undefined);
    const membership: Record<string, unknown> = { role: 'member', save };
    const { service, updateOne } = createService(membership);

    await service.removeMember(WS_ID, USER_ID);

    expect(membership.isDeleted).toBeInstanceOf(Date);
  ...
  it('soft delete와 membersCount 감소를 한 트랜잭션으로 묶는다', async () => {
    ...
    expect(save).toHaveBeenCalledWith({ session: FAKE_SESSION });
  });

  it('추방 대상 조회 실패는 트랜잭션을 열지 않는다', async () => {
    const { service, withTransaction } = createService(null);
