...
export function registerCreateFolder(server: McpServer, client: ApiClient) {
  server.registerTool(
    "create_folder",
    {
      title: "폴더 생성",
      description:
        "Mock API를 계층적으로 분류하기 위한 폴더를 생성합니다. " +
        "중간 경로의 폴더가 없으면 상위부터 자동으로 일괄 생성하며, 이미 존재하는 폴더는 유지되므로 멱등(Idempotent)하게 안전하게 호출할 수 있습니다.",
      annotations: {
      ...
      inputSchema: {
        path: z
          .string()
          .describe(
            "생성할 폴더의 절대 경로 (루트 '/'는 항상 존재하므로 제외, 예: /shop/v1)"
          ),
      },
    },
