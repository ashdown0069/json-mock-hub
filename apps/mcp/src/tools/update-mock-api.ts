...
export function registerUpdateMockApi(
  ...
  server.registerTool(
    "update_mock_api",
    {
      title: "Mock API 갱신",
      description:
        "경로로 지정한 mock API의 mock 데이터를 새로 생성해 갱신합니다(이름·위치·미지정 옵션은 유지). " +
        "필드를 추가·수정하려면 먼저 describe_mock_api로 저장된 스키마를 확인한 뒤, 그 스키마에 변경을 얹어 schema로 넘기세요. " +
        "schema를 생략하면 저장된 스키마를 그대로 재사용합니다. 경로는 list_mock_apis로 확인하세요. " +
      ...
      inputSchema: {
        ...
        schema: schemaObjectInput
          .optional()
          .describe(
            "생략하면 저장된 스키마를 그대로 재사용합니다. 지정하면 기존 스키마를 완전히 대체합니다."
          ),
        count: z
          .number()
          .int()
