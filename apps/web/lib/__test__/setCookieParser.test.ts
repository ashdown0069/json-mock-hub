...
describe("parseSetCookieValue", () => {
  it("Set-Cookie 라인들에서 대상 이름의 쿠키 값을 추출한다", () => {
    const lines = [
      "__Host-refresh-token=r1; Path=/; Secure; HttpOnly",
      "__Host-access-token=a1; Path=/; Secure; HttpOnly; SameSite=Strict",
    ]
  ...
  it("URL 인코딩된 값을 디코딩한다", () => {
    const lines = ["__Host-access-token=a%3D1; Path=/; Secure"]
    expect(parseSetCookieValue(lines, ACCESS_TOKEN_COOKIE)).toBe("a=1")
  })

  it("값이 빈 문자열이면 null을 반환한다", () => {
    const lines = ["__Host-access-token=; Path=/; Secure"]
